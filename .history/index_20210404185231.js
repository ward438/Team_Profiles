...
function getSavedData(filename, objectType) {
    let data = read(filename);
    let objectArray = [];
    data.forEach(item => {
        let tmpItem = new objectType(input);
        tmpItem.buildFromJson(item);
        objectArray.push(tmpItem);
    })
    return objectArray;
}

// ________________________________________________________
let employees = getSavedData('employees.json', Employee);
let managers = getSavedData('managers.json', Manager);
let interns = getSavedData('interns.json', Intern);
let engineers = getSavedData('engineers.json', Engineer);
// ________________________________________________________
                ...
                case '4':
                    {
                        ...
                        write(employees, 'employees.json')
                        write(managers, 'managers.json')
                        write(engineers, 'engineers.json')
                        write(interns, 'interns.json')
